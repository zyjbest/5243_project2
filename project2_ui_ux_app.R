      ...
      tabItem(
        ...
        fluidRow(
          column(
            4,
            box(
              width = 12, title = "Create New Feature",
              status = "primary", solidHeader = TRUE,
              selectInput(
            ...
            box(
              width = 12, title = "Created Features",
              status = "info", solidHeader = TRUE, collapsible = TRUE,
          ...
          column(
            8,
            box(
              width = 12, title = "Data Preview with New Features",
              status = "success", solidHeader = TRUE,
              tabsetPanel(
...
server <- function(input, output, session) {
  ...
  observeEvent(loaded_data(), {
    ...
    log_op("Dataset loaded successfully.")
    
    showNotification(
      sprintf("Loaded successfully: %d rows × %d columns", nrow(df), ncol(df)),
      type = "message",
      duration = 4
    )
  })
